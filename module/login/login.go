...
// Modificar getModulesForRole para recibir rolID como uint64
func getModulesForRole(rolID uint64) []ModuleAccess {
	var modules []ModuleAccess
	var rolModulos []RolModulo

	// Consulta optimizada con preload
	if err := database.DBconn.Preload("Modulo").Where("rol_id = ? AND acceso = ?", rolID, true).Find(&rolModulos).Error; err != nil {
		return modules
	}
	...
	for _, rm := range rolModulos {
		if rm.Modulo != nil {
			modules = append(modules, ModuleAccess{
				ModuloID: uint64(rm.Modulo.ID),
				Codigo:   rm.Modulo.Codigo,
...
// Helper: Obtener accesos a empresas para un ID de usuario con preload.
func getCompanyAccesses(usuarioID uint) []CompanyAccess {
	var accesos []CompanyAccess
	var ueList []UsuarioEmpresa

	// Consulta optimizada con preload
	if err := database.DBconn.Preload("Rol").Where("usuario_id = ?", usuarioID).Find(&ueList).Error; err != nil {
		return accesos
	}
	...
	for _, ue := range ueList {
		if ue.Rol != nil {
			accesos = append(accesos, CompanyAccess{
				...
				Role: RoleAccess{
					RolID:   uint64(ue.Rol.ID),
					Codigo:  ue.Rol.Codigo,
					Nombre:  ue.Rol.Nombre,
					Modules: getModulesForRole(uint64(ue.Rol.ID)), // conversión para consistencia
				},
			})
		}
	}
...
func loginService(req LoginRequest, ip string) (fiber.Map, string, error) {
	...
	accesos := getCompanyAccesses(usuario.ID)

	// Configurar los claims del token JWT con más seguridad.
	claims := TokenClaims{
