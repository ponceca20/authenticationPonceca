...
func (r *usuarioRepo) UpdateUsuario(id uint64, updatedUsuario *Usuario) (Usuario, error) {
	var usuario Usuario
	// Eliminamos el uso de Transaction para optimizar la consulta
	tx := r.DB.Model(&Usuario{}).Where("id = ?", id).Updates(updatedUsuario)
	if tx.Error != nil {
		return usuario, fmt.Errorf("error al actualizar usuario con id %d: %w", id, tx.Error)
	}
...
func (r *usuarioRepo) DeleteUsuario(id uint64) error {
	// Eliminación directa sin Transaction
	tx := r.DB.Delete(&Usuario{}, id)
	if tx.Error != nil {
		return fmt.Errorf("error al eliminar usuario: %w", tx.Error)
	}
	if tx.RowsAffected == 0 {
...
func (r *usuarioRepo) PatchUsuario(id uint64, fields map[string]interface{}) (Usuario, error) {
	var usuario Usuario
	// Actualización directa sin Transaction
	tx := r.DB.Model(&Usuario{}).Where("id = ?", id).Updates(fields)
	if tx.Error != nil {
		return usuario, fmt.Errorf("error al actualizar parcialmente usuario con id %d: %w", id, tx.Error)
	}
	if tx.RowsAffected == 0 {
		return usuario, fmt.Errorf("usuario con id %d no encontrado", id)
	}
	// Recuperación del registro actualizado
	err := r.DB.First(&usuario, id).Error
	if err != nil {
		return usuario, fmt.Errorf("error al recuperar usuario con id %d: %w", id, err)
	}
